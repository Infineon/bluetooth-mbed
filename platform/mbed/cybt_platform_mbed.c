/*******************************************************************************
* \file cybt_platform_mbed.c
*
* \brief
* Implementation for BT porting interface on MBED
*
********************************************************************************
* \copyright
...
#define MBED_SLEEP_TASK_NAME               "mbed_sleep_task"
#define MBED_SLEEP_TASK_STACK_SIZE         (1024)
#define MBED_SLEEP_TASK_PRIORITY           (CY_RTOS_PRIORITY_REALTIME)
#define MBED_SLEEP_TASK_QUEUE_COUNT        (32)
#define MBED_SLEEP_TASK_QUEUE_ITEM_SIZE    (sizeof(uint8_t))

/*****************************************************************************
 *                           Type Definitions
 *****************************************************************************/
#define MBED_SLEEP_ACT_START_IDLE_TIMER    (0x20)
#define MBED_SLEEP_ACT_STOP_IDLE_TIMER     (0x40)
typedef uint8_t mbed_sleep_action_t;
...
cy_thread_t     mbed_sleep_timer_task;
cy_queue_t      mbed_sleep_timer_task_queue;
...
cybt_result_t cybt_send_action_to_mbed_sleep_task(mbed_sleep_action_t action);
void cybt_mbed_sleep_timer_task(cy_thread_arg_t arg);
...
void cybt_platform_init(void)
{
    ...
    cy_rtos_init_queue(&mbed_sleep_timer_task_queue,
                       MBED_SLEEP_TASK_QUEUE_COUNT,
                       MBED_SLEEP_TASK_QUEUE_ITEM_SIZE
                      );

    cy_rtos_create_thread(&mbed_sleep_timer_task,
                          cybt_mbed_sleep_timer_task,
                          MBED_SLEEP_TASK_NAME,
                          NULL,
                          MBED_SLEEP_TASK_STACK_SIZE,
                          MBED_SLEEP_TASK_PRIORITY,
                          (cy_thread_arg_t) 0
                         );
}
...
void cybt_platform_deinit(void)
{
    ...
    cy_rtos_terminate_thread((cy_thread_t *)&mbed_sleep_timer_task);

    cy_rtos_deinit_queue(&mbed_sleep_timer_task_queue);
...
void cybt_platform_sleep_lock(void)
{
    ...
    cybt_send_action_to_mbed_sleep_task(MBED_SLEEP_ACT_STOP_IDLE_TIMER);
...
void cybt_platform_sleep_unlock(void)
{
    ...
    {
        cybt_send_action_to_mbed_sleep_task(MBED_SLEEP_ACT_START_IDLE_TIMER);
        cybt_platform_enable_irq();
    }
    else
    {
...
void cybt_idle_timer_cback(cy_timer_callback_arg_t arg)
{
    cyhal_uart_event_t disable_irq_event = (CYHAL_UART_IRQ_RX_DONE
                                            | CYHAL_UART_IRQ_TX_DONE
                                            | CYHAL_UART_IRQ_RX_NOT_EMPTY
                                           );
...
cybt_result_t cybt_send_action_to_mbed_sleep_task(mbed_sleep_action_t action)
{
    bool is_from_isr = core_util_is_isr_active();
    cy_rslt_t result = cy_rtos_put_queue(&mbed_sleep_timer_task_queue,
                                         (void *)&action,
                                         0,
                                         is_from_isr
                                        );
...
// This task may be specific to MBED OS because the timer is unable to
// be started/stopped in interrupt context and irq masked mode.
void cybt_mbed_sleep_timer_task(cy_thread_arg_t arg)
{
    cy_rslt_t result;
    mbed_sleep_action_t action = 0;

    while(1)
    {
        result = cy_rtos_get_queue(&mbed_sleep_timer_task_queue,
                                   (void *)&action,
        ...
        if(CY_RSLT_SUCCESS != result)
        {
            MAIN_TRACE_WARNING("mbed_sleep_task(): queue error (0x%x)",
                                result
                              );
            continue;
        }
        ...
        switch(action)
        {
            ...
            case MBED_SLEEP_ACT_STOP_IDLE_TIMER:
                ...
                break;
            default:
                MAIN_TRACE_ERROR("mbed_sleep_task(): Unknown action = 0x%x", action);
                break;
        }
    }
}
